...
func isSitemap(urls []string) ([]string, []string) {
	//check if the urlLink is a sitemap url or web page url
	sitemapFiles := []string{}
	pages := []string{}
	...
	for ; n > 0; n-- {
		...
		for _, link := range list {
			...
			go func(link string) {
				...
				// now separating sitemap files and normal web pages from extracted urls
				sitemapFiles, pages := isSitemap(urls)
				if sitemapFiles != nil {
					worklist <- sitemapFiles
				}
				//then we will go to each url present in sitemap url file
...
func extractUrls(response *http.Response) ([]string, error) {
	...
	results := []string{}
	//xml files contain loc tags containing info.
	// so returning all text of those nodes/loc
	sel := doc.Find("loc")
...
func ScrapeSitemap(url string, parser Parser, concurrency int) []SeoData {
	//results is all  urlLinks present in sitemap files
	//they are important because included in sitemap files, for search engines to crawl
	results := extractSitemapURLs(url)
	//now , we will crawl those urls which were found in sitemap files and scrape SEO data
...
func main() {
	p := DefaultParser{}
	results := ScrapeSitemap("https://www.sdlccorp.com/sitemap.xml", p, 10)
